...
export class PurchaseOrderController {
	...
	private static async emailPDF(purchaseOrderId) {
		const { data: purchaseOrderData } = await this.getOne({
			id: purchaseOrderId,
		});

		const supplier = purchaseOrderData.quotation.quotationRequest.supplier;
	...
	static async update(data) {
		...
		const selectedEntry = await getRepository(PurchaseOrder)
			.findOne(data.id)
			.catch((e) => {
				console.log(e.code, e);
				throw {
		...
		try {
			...
			await getRepository(PurchaseOrderMaterial)
				.createQueryBuilder()
				.delete()
	...
	static async delete({ id }) {
		...
		const entry = await getRepository(PurchaseOrder)
			.findOne({ id: id })
			.catch((e) => {
				console.log(e.code, e);
				throw {
		...
		// find deleted status
		const deletedStatus = await getRepository(PurchaseOrderStatus)
		...
		await getRepository(PurchaseOrder)
			.save(entry)
			.catch((e) => {
				console.log(e.code, e);
				throw {
	...
	static async getSupplierPurchaseOrders({
		...
		let entires = await PurchaseOrderDao.getSupplierPurchaseOrders(
			supplierId,
			purchaseOrderStatusName
		).catch((e) => {
			console.log(e.code, e);
