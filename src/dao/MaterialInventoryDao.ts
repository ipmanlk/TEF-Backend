...
export class MaterialInventoryDao {
	...
	static async updateInventoryStatuses() {
		...
		const lowStatus = await getRepository(MaterialInventoryStatus).findOne({
			where: { name: "Low" },
		});
		const normalStatus = await getRepository(MaterialInventoryStatus).findOne({
			where: { name: "Normal" },
		});
		...
		for (let material of materials) {
			...
			if (parseFloat(inventoryMaterial.availableQty) < material.rop) {
				...
				inventoryMaterial.materialInventoryStatus = normalStatus;
			}
